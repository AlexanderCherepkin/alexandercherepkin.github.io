/* ===== /remont/js/one-room.js v2 — оптимизация производительности ===== */
(() => {
const $ = (s, r = document) => r.querySelector(s);
const $$ = (s, r = document) => Array.from(r.querySelectorAll(s));

/* ---------- Lightbox styles autoinject ---------- */
function ensureLBStyles() {
if (document.getElementById('lb-autostyles')) return;
const css = `.lightbox{position:fixed;inset:0;z-index:1500;display:grid;place-items:center}.lightbox[hidden]{display:none!important}.lightbox__backdrop{position:absolute;inset:0;background:rgba(2,6,23,.75);backdrop-filter:blur(.2rem)}.lightbox__frame{position:relative;max-width:92vw;max-height:92dvh;display:grid;place-items:center;outline:.16rem solid color-mix(in oklch,#166534 50%,white);border-radius:1rem;box-shadow:0 .8rem 2rem rgba(0,0,0,.4);overflow:hidden;background:#0b1220}.lightbox__img{max-width:100%;max-height:92dvh;display:block}.lightbox__close,.lightbox__nav{position:absolute;z-index:2;display:grid;place-items:center;min-width:48px;min-height:48px;border-radius:.8rem;background:rgba(255,255,255,.12);color:#fff;border:.1rem solid rgba(255,255,255,.2);cursor:pointer;transition:transform .15s ease,background .15s ease;touch-action:manipulation}.lightbox__close:hover,.lightbox__nav:hover{background:rgba(255,255,255,.2);transform:translateY(-.06rem)}.lightbox__close{top:.6rem;right:.6rem}.lightbox__nav{top:50%;transform:translateY(-50%)}.lightbox__prev{left:1rem}.lightbox__next{right:1rem}.lightbox__svg{width:2rem;height:2rem;pointer-events:none}`;
...
const hasStableGutter = getComputedStyle(document.documentElement).scrollbarGutter?.includes('stable');
...
window.addEventListener('pagehide', unlockScroll, { passive: true });
...
lb.hidden = true;
lb.innerHTML = `<div class="lightbox__backdrop" data-lb-close></div><figure class="lightbox__frame" role="dialog" aria-modal="true" aria-label="Просмотр изображения"><button class="lightbox__close" type="button" data-lb-close aria-label="Закрыть"><svg class="lightbox__svg" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M18.3 5.71 12 12l6.3 6.29-1.41 1.42L10.59 13.4 4.29 19.7 2.88 18.3 9.17 12 2.88 5.71 4.29 4.29l6.3 6.3 6.29-6.3 1.42 1.42Z"></path></svg></button><button class="lightbox__nav lightbox__prev" type="button" data-lb-prev aria-label="Предыдущее"><svg class="lightbox__svg" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12z"></path></svg></button><img class="lightbox__img" alt=""><button class="lightbox__nav lightbox__next" type="button" data-lb-next aria-label="Следующее"><svg class="lightbox__svg" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="m8.59 16.59 1.41 1.41 6-6-6-6-1.41 1.41L13.17 12z"></path></svg></button></figure>`;
...
// Фокусируемость превью
function enhance(root = document) {
$$('.case-gallery img[data-full]', root).forEach((img) => {
...
if (n.matches?.('.case-gallery')) enhance(n);
else enhance(n);
});
});
});
...
const hasGSAP = typeof window.gsap !== 'undefined' && typeof window.gsap.fromTo === 'function';
let kb = null;

function ken(img) {
if (!img) return;
if (hasGSAP) {
if (kb && typeof kb.kill === 'function') kb.kill();
kb = window.gsap.fromTo(
img,
{ scale: 1.008 },
{ scale: 1.02, duration: interval / 1000, ease: 'none', overwrite: 'auto' }
);
} else {
...
d.setAttribute('aria-label', `Перейти к слайду ${k + 1} из ${totalSlides}`);
});
slides.forEach((s, k) => s.setAttribute('aria-label', `${k + 1} из ${totalSlides}`));
i = idx;
progress();
...
s.textContent = '.slides *{animation:none!important;transition:none!important}';
