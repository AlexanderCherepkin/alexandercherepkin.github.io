...
(() => {
  ...
  function ensureLBStyles(){
    ...
    const css = `
      ...
      .lightbox__close,.lightbox__nav{position:absolute;z-index:2;display:grid;place-items:center;width:3.4rem;height:3.4rem;border-radius:.8rem;background:rgba(255,255,255,.12);color:#fff;border:.1rem solid rgba(255,255,255,.2);cursor:pointer;transition:transform .15s ease,background .15s ease}
      .lightbox__close:hover,.lightbox__nav:hover{background:rgba(255,255,255,.2);transform:translateY(-.06rem)}
  ...
  /* ---------- Scroll lock (учёт stable gutter) ---------- */
  ...
  new MutationObserver(muts=>{
    muts.forEach(m=>{
      m.addedNodes && m.addedNodes.forEach(n=>{
        ...
        if (n.matches?.('.case-gallery')) enhance(n); else enhance(n);
      });
    });
  ...
  if (root){
    ...
    const interval = Math.max(3000, Number(root.dataset.interval || 8000)); // 8 сек по умолчанию
    let t=null;
    ...
    function progress(){
      barInner.style.transition='none'; barInner.style.width='0%';
      requestAnimationFrame(()=>{ barInner.style.transition=`width ${interval}ms linear`; barInner.style.width='100%'; });
    }
    const hasGSAP = typeof window.gsap !== 'undefined';
    let kb=null;
    function ken(img){
      if (!img) return;
      if (hasGSAP){ kb?.kill(); kb = gsap.fromTo(img,{scale:1.008},{scale:1.02,duration:interval/1000,ease:'none',overwrite:'auto'}); }
      else { img.style.transition=`transform ${interval}ms linear`; img.style.transform='translateZ(0) scale(1.02)'; setTimeout(()=>{img.style.transition='none'; img.style.transform='translateZ(0) scale(1)';}, interval); }
    }
    function setActive(idx){
      slides.forEach((s,k)=> s.classList.toggle('is-active',k===idx));
      dots.forEach((d,k)=>{ d.classList.toggle('is-active',k===idx); d.setAttribute('aria-selected', String(k===idx)); });
      slides.forEach((s,k)=> s.setAttribute('aria-label',`${k+1} из ${slides.length}`));
      i=idx; progress(); if (root.dataset.overlay!=='on') ken(slides[i]?.querySelector('img'));
    }
    const nextSlide=()=> setActive((i+1)%slides.length);
    const prevSlide=()=> setActive((i-1+slides.length)%slides.length);
    function start(){ if (autoplay && !t) t=setInterval(nextSlide, interval); }
    ...
    if (m.matches){
      root.dataset.autoplay='false'; root.dataset.paused='true'; stop();
      const s=document.createElement('style'); s.textContent='.slides *{animation:none!important;transition:none!important}'; document.head.appendChild(s);
    }
  }
  ...
  if (toTop){
    ...
    const onScroll = ()=>{ toTop.style.opacity = window.scrollY>thr ? '1' : '0'; };
    onScroll(); window.addEventListener('scroll', onScroll, {passive:true});
    toTop.addEventListener('click', ()=> window.scrollTo({top:0,behavior:'smooth'}));
  }

  // Инициализация AOS (если подключён)
  document.addEventListener('DOMContentLoaded', ()=> { if (window.AOS) AOS.init({ once:true, offset:80, duration:600 }); });
})();
